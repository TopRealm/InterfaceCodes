 ...
 *
 * @source <https://git.qiuwen.wiki/InterfaceAdmin/Gadgets/src/branch/master/src/Gadgets/ShortURL>
...
(function shortURL() {
  ...
  var doIns = function doIns(link) {
    ...
    element.firstElementChild.onclick = function (event) {
      ...
      var $element = $('<div>');
      for (var _i = 0, _arr = ['youshou.wiki', /* 'qwbk.org', 'bkwz.cn'*/]; _i < _arr.length; _i++) {
        var domain = _arr[_i];
    ...
    if (!headerElement) {
      var $headerElement = $('<div>').attr('id', 'mw-indicator-shortURL').addClass('mw-indicator').append($('<a>').attr({
        href: '#',
        title: text
    ...
    headerElement.onclick = function (event) {
      ...
      event.preventDefault();
      var shorturl = "https://youshou.wiki".concat(link);
      // eslint-disable-next-line compat/compat
      (_navigator$clipboard = navigator.clipboard) === null || _navigator$clipboard === void 0 ? void 0 : _navigator$clipboard.writeText(shorturl);
      mw.notify($('<span>').text(title).append($('<br>')).append($('<a>').attr('href', '#').text(shorturl).on('click', function (event_) {
