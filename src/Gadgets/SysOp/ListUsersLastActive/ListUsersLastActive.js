...
(function listUsersLastActive() {
  ...
  var addLeadingZero = function addLeadingZero(number) {
    ...
    if (typeof number === 'number' && number < 10) {
      numberString = "0".concat(number.toString());
    } else {
      numberString = number.toString();
    }
    return numberString;
  };
  var adjustTime = function adjustTime(time) {
    var date;
    var today = new Date();
    ...
    var utcOffsetToString;
    if (utcOffset >= 0) {
      utcOffsetToString = "+".concat(utcOffset);
    } else {
      utcOffsetToString = "\u2212".concat(Math.abs(utcOffset));
    }
    // set the date bits to output
    var year = time.getFullYear();
    var month = addLeadingZero(time.getMonth() + 1);
    var day = time.getDate();
    var hour = Number.parseInt(time.getHours().toString());
    ...
    var hourMinute = "".concat(addLeadingZero(hour), ":").concat(minute, " ");
    var returnDate = "".concat(date, ", ").concat(hourMinute, " (UTC").concat(utcOffsetToString, ")");
    return returnDate;
  };
  var checkLastActive = /*#__PURE__*/function () {
    var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(name, element) {
      var api, maxdate, usercontribs, logevents, date, message;
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          ...
          case 8:
            logevents = _context.sent;
            if (logevents['query'].logevents[0] !== undefined) {
              date = new Date(logevents['query'].logevents[0].timestamp);
              if (maxdate === null) {
                maxdate = date;
            ...
            if (maxdate === null) {
              message = "\uFF08".concat(window.wgULS('从未有操作', '從未有動作'), "\uFF09");
            } else {
              message = "\uFF08".concat(window.wgULS('最后操作于', '最後動作於')).concat(adjustTime(maxdate), "\uFF09");
            }
    ...
    return function checkLastActive(_x, _x2) {
      return _ref.apply(this, arguments);
    };
  }();
  $('#mw-content-text>ul>li').each(function (_index, element) {
