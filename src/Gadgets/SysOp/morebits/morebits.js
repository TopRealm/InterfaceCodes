...
(function (window, document, $) {
  ...
  Morebits.isPageRedirect = function () {
    return !!(mw.config.get('wgIsRedirect') || document.querySelector('#softredirect') || $('.box-RfD').length || $('.box-Redirect_category_shell').length);
  };
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        if (element instanceof Node) {
          ...
          $.parseHTML(Morebits.createHtml.renderWikilinks(element)).forEach(function (node) {
  ...
  Morebits.createHtml.renderWikilinks = function (text) {
    ...
    ub.content = ub.content.replace(/\[\[:?(?:([^\]|]+?)\|)?([^\]|]+?)]]/g, function (_, target, text) {
      ...
      return "<a target=\"_blank\" href=\"".concat(mw.util.getUrl(target), "\" title=\"").concat(target.replace(/"/g, '&#34;'), "\">").concat(text, "</a>");
    });
    return ub.rebind();
  };
  ...
  Morebits.quickForm.element.prototype.append = function QuickFormElementAppend(data) {
    var child = data instanceof Morebits.quickForm.element ? data : new Morebits.quickForm.element(data);
  ...
  Morebits.quickForm.element.prototype.compute = function QuickFormElementCompute(data, in_id) {
    ...
    switch (data.type) {
      ...
      case 'dyninput':
        {
          var min = data.min || 1;
          var max = data.max || Number.POSITIVE_INFINITY;
          node = document.createElement('div');
  ...
  Morebits.ip = {
    ...
    validCIDR: function validCIDR(ip) {
      if (Morebits.ip.isRange(ip)) {
        var subnet = Number.parseInt(ip.match(/\/(\d{1,3})$/)[1], 10);
        if (subnet) {
          // Should be redundant
    ...
    get64: function get64(ipv6) {
      ...
      var ip_re = /^((?:[\dA-F]{1,4}:){4})(?:[\dA-F]{1,4}:){3}[\dA-F]{1,4}(?:\/\d{1,3})?$/;
  ...
  Morebits.string = {
    ...
    splitWeightedByKeys: function splitWeightedByKeys(str, start, end, skiplist) {
      ...
      for (var i = 0; i < str.length; ++i) {
        var _iterator2 = _createForOfIteratorHelper(skiplist),
          _step2;
        try {
     ...
     * @returns {string}
     */
    safeReplace: function safeReplace(string, pattern, replacement) {
     ...
     * @returns {boolean}
     */
    isInfinity: function isInfinity(expiry) {
  ...
  Morebits.unbinder.getCallback = function (self) {
  ...
  Morebits.date.prototype = {
    ...
    add: function add(number, unit) {
      var num = Number.parseInt(number, 10); // normalize
      if (Number.isNaN(num)) {
  ...
  Morebits.wiki.api = function (currentAction, query, onSuccess, statusElement, onError) {
    ...
    (_this$query = this.query).uselang || (_this$query.uselang = 'content'); // Use wgUserLanguage for preview
  ...
  Morebits.wiki.api.prototype = {
    ...
    post: function post(callerAjaxParameters) {
      ...
      function onAPIfailure(error, statusText, errorThrown) {
        ...
        this.errorText = "".concat(statusText, "\u5728\u8C03\u7528API\u65F6\u53D1\u751F\u4E86\u9519\u8BEF\u201C").concat(error.statusText, "\u201D");
  ...
  Morebits.wiki.page = function (pageName, status) {
    ...
    this.save = function (onSuccess, onFailure) {
      ...
      if (ctx.fullyProtected && !ctx.suppressProtectWarning && !confirm(ctx.fullyProtected === 'infinity' ? "\u60A8\u5373\u5C06\u7F16\u8F91\u5168\u4FDD\u62A4\u9875\u9762\u201C".concat(ctx.pageName, "\u201D\uFF08\u6C38\u4E45\uFF09\u3002\n\n\u5355\u51FB\u201C\u786E\u5B9A\u201D\u4EE5\u786E\u8BA4\u64CD\u4F5C\uFF0C\u6216\u5355\u51FB\u201C\u53D6\u6D88\u201D\u4EE5\u53D6\u6D88\u64CD\u4F5C\u3002") : "You are about to make an edit to the fully protected page \"".concat(ctx.pageName, "\"\uFF08\u4FDD\u62A4\u671F\u81F3\uFF1A").concat(new Morebits.date(ctx.fullyProtected).calendar('utc'), " (UTC)\uFF09\u3002\n\n\u5355\u51FB\u201C\u786E\u5B9A\u201D\u4EE5\u786E\u8BA4\u64CD\u4F5C\uFF0C\u6216\u5355\u51FB\u201C\u53D6\u6D88\u201D\u4EE5\u53D6\u6D88\u64CD\u4F5C\u3002"))) {
    ...
    var fnLoadSuccess = function fnLoadSuccess() {
      ...
      if (ctx.editMode === 'revert') {
        ...
        ctx.editSummary = "[[QW:UNDO|\u64A4\u9500]]\u7531 ".concat(ctx.revertUser, " \u6240\u505A\u51FA\u7684") + "\u4FEE\u8BA2 ".concat(ctx.revertOldID, "\uFF1A").concat(ctx.editSummary);
      }
      ctx.pageLoaded = true;
    ...
    var fnCheckPageName = function fnCheckPageName(response, onFailure) {
      ...
      if (page) {
        ...
        if (page.invalid) {
          ctx.statusElement.error("\u6807\u9898\u4E0D\u5408\u6CD5\uFF1A".concat(ctx.pageName));
          onFailure(this);
          return false; // abort
        }
        ...
        if (response.redirects) {
          ...
          if (origNs !== newNs && !ctx.followCrossNsRedirect) {
            ctx.statusElement.error("".concat(ctx.pageName, "\u662F\u8DE8\u547D\u540D\u7A7A\u95F4\u91CD\u5B9A\u5411\u5230").concat(resolvedName, "\uFF0C\u7565\u8FC7"));
            onFailure(this);
            return false;
          }

          // only notify user for redirects, not normalization
          var info = new Morebits.status('信息', "\u4ECE ".concat(ctx.pageName, " \u91CD\u5B9A\u5411\u5230 ").concat(resolvedName));
          info();
        }
        ctx.pageName = resolvedName; // update to redirect target or normalized name
      } else {
        // could be a circular redirect or other problem
        ctx.statusElement.error("\u4E0D\u80FD\u89E3\u6790\u9875\u9762\u7684\u91CD\u5B9A\u5411\uFF1A".concat(ctx.pageName));
        onFailure(this);
      ...
      if (errorCode === 'editconflict' && ctx.conflictRetries++ < ctx.maxConflictRetries) {
        ...
        switch (errorCode) {
          ...
          case 'abusefilter-disallowed':
            {
              ctx.statusElement.error("\u7F16\u8F91\u88AB\u9632\u6EE5\u7528\u8FC7\u6EE4\u5668\u89C4\u5219\u201C".concat(errorData.abusefilter.description, "\u201D\u963B\u6B62\uFF1B\u82E5\u60A8\u8BA4\u4E3A\u60A8\u7684\u8BE5\u6B21\u7F16\u8F91\u662F\u6709\u610F\u4E49\u7684\uFF0C\u8BF7\u81F3\u201CQiuwen:\u9632\u6EE5\u7528\u8FC7\u6EE4\u5668\u201D\u63D0\u62A5\u3002"));
              break;
            }
          case 'abusefilter-warning':
            {
              ctx.statusElement.error(['编辑被防滥用过滤器规则“', errorData.abusefilter.description, '”警告；若您仍希望做出该编辑，请尝试重新提交——根据过滤器的设置，您可能仍可以作出此编辑；若您认为您的该次编辑是有意义的，请至“Qiuwen:防滥用过滤器”提报。']);
          ...
          case 'spamblacklist':
            {
              ...
              ctx.statusElement.error("\u4E0D\u80FD\u4FDD\u5B58\u9875\u9762\uFF0C\u56E0URL ".concat(spam, " \u5728\u5783\u573E\u94FE\u63A5\u9ED1\u540D\u5355\u4E2D\u3002"));
              break;
            }
          default:
            {
              ctx.statusElement.error("\u4E0D\u80FD\u4FDD\u5B58\u4FEE\u6539\uFF1A".concat(ctx.saveApi.getErrorText()));
            }
        }
        ctx.editMode = 'all'; // cancel append/prepend/newSection/revert modes
    ...
    var fnLookupCreationSuccess = function fnLookupCreationSuccess() {
      ...
      if (!rev) {
        ctx.statusElement.error("\u65E0\u6CD5\u627E\u5230".concat(ctx.pageName, "\u7684\u4EFB\u4F55\u4FEE\u8BA2\u7248\u672C"));
        ctx.onLookupCreationFailure(this);
        return;
      }
    ...
    var fnLookupNonRedirectCreator = function fnLookupNonRedirectCreator() {
      ...
      var _iterator3 = _createForOfIteratorHelper(revs),
        _step3;
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          var rev = _step3.value;
    ...
    var fnPreflightChecks = function fnPreflightChecks(action, onFailure) {
      ...
      if (!Morebits.userIsSysop && action !== 'move') {
        ctx.statusElement.error("\u65E0\u6CD5\u5BF9\u9875\u9762\u8FDB\u884C\u201C".concat(action, "\u201D\u64CD\u4F5C\uFF1A\u53EA\u6709\u7BA1\u7406\u5458\u53EF\u4EE5\u8FDB\u884C\u6B64\u64CD\u4F5C"));
        onFailure(this);
        return false;
      }
      if (!ctx.editSummary) {
    ...
    var fnProcessChecks = function fnProcessChecks(action, onFailure, response) {
      ...
      if (actionMissing || protectMissing || saltMissing) {
        ctx.statusElement.error("\u65E0\u6CD5\u5BF9\u9875\u9762\u8FDB\u884C\u201C".concat(action, "\u201D\u64CD\u4F5C\uFF0C\u56E0\u4E3A\u9875\u9762").concat(missing ? '已不' : '已经', "\u5B58\u5728"));
        onFailure(this);
        return false;
      }
      ...
      if (editprot && !ctx.suppressProtectWarning && !confirm("\u60A8\u5373\u5C06\u5BF9\u5168\u4FDD\u62A4\u9875\u9762\u201C".concat(ctx.pageName).concat(editprot.expiry === 'infinity' ? '”（永久）' : "\u201D\uFF08\u5230\u671F\uFF1A".concat(new Morebits.date(editprot.expiry).calendar('utc'), " (UTC)\uFF09"), "\u201D\u8FDB\u884C\u201C").concat(action, "\u201D\u64CD\u4F5C") + '。\n\n单击确定以继续操作，或单击取消以取消操作。')) {
    ...
    var fnProcessDeleteError = function fnProcessDeleteError() {
      ...
      if (errorCode === 'internal_api_error_DBQueryError' && ctx.retries++ < ctx.maxRetries) {
        ...
        ctx.statusElement.error("\u65E0\u6CD5\u5220\u9664\u9875\u9762\uFF1A".concat(ctx.deleteProcessApi.getErrorText()));
        if (ctx.onDeleteFailure) {
          ctx.onDeleteFailure.call(this, ctx.deleteProcessApi); // invoke callback
        }
      }
    ...
    var fnProcessUndeleteError = function fnProcessUndeleteError() {
      ...
      if (errorCode === 'internal_api_error_DBQueryError') {
        ...
        ctx.statusElement.error("\u65E0\u6CD5\u8FD8\u539F\u9875\u9762\uFF1A".concat(ctx.undeleteProcessApi.getErrorText()));
        if (ctx.onUndeleteFailure) {
          ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi); // invoke callback
        }
      }
    ...
    var fnProcessProtect = function fnProcessProtect() {
      ...
      if (ctx.protectCascade) {
        ...
        if ((!ctx.protectEdit || ctx.protectEdit.level !== 'sysop' || !ctx.protectMove || ctx.protectMove.level !== 'sysop') && !confirm("\u60A8\u5DF2\u5BF9\u201C".concat(ctx.pageName, "\u201D\u542F\u7528\u4E86\u8FDE\u9501\u4FDD\u62A4") + '，但没有设置仅管理员的保护级别。\n\n' + '单击确认以自动调整并继续连锁全保护，单击取消以跳过此操作')) {
  ...
  Morebits.wikitext.parseTemplate = function (text, start) {
     ...
     */
    var findParam = function findParam(final) {
    ...
    for (var i = start; i < text.length; ++i) {
      var test3 = text.slice(i, i + 3); // text.substr(i, 3)
      ...
      var test2 = text.slice(i, i + 2); // text.substr(i, 2)
  ...
  Morebits.wikitext.page.prototype = {
    ...
    commentOutImage: function commentOutImage(image, reason) {
      ...
      try {
        for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
          var allLink = _step4.value;
          if (links_re.test(allLink)) {
            var replacement = "<!-- ".concat(reason).concat(allLink, " -->");
    ...
    addToImageComment: function addToImageComment(image, data) {
      ...
      var _iterator5 = _createForOfIteratorHelper(allLinks),
        _step5;
      try {
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
          var replacement = _step5.value;
    ...
    removeTemplate: function removeTemplate(template) {
      ...
      var _iterator6 = _createForOfIteratorHelper(allTemplates),
        _step6;
      try {
        for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
          var allTemplate = _step6.value;
    ...
    insertAfterTemplates: function insertAfterTemplates(tag, regex, flags, preRegex) {
      ...
      preRegex, "|") + // begin template format
      "\\{\\{\\s*(?:".concat(
      // Template regex
      regex
  ...
  Morebits.userspaceLogger = function (logPageName) {
    ...
    this.log = function (logText, summaryText) {
      ...
      var page = new Morebits.wiki.page("User:".concat(mw.config.get('wgUserName'), "/").concat(logPageName), '将项目加入到用户空间日志'); // make this '... to ' + logPageName ?
      page.load(function (pageobj) {
        // add blurb if log page doesn't exist or is blank
        var text = pageobj.getPageText() || _this2.initialText;

        // create monthly header if it doesn't exist already
        var date = new Morebits.date(pageobj.getLoadTime());
        if (!date.monthHeaderRegex().test(text)) {
          text += "\n\n".concat(date.monthHeader(_this2.headerLevel));
        }
        pageobj.setPageText("".concat(text, "\n").concat(logText));
        pageobj.setEditSummary(summaryText);
        pageobj.setChangeTags(_this2.changeTags);
  ...
  Morebits.batchOperation = function (currentAction) {
    ...
    this.workerSuccess = function (arg) {
      if (arg instanceof Morebits.wiki.api || arg instanceof Morebits.wiki.page) {
        ...
        var info = new Morebits.status(arg, "\u5B8C\u6210\uFF08[[".concat(arg, "]]\uFF09"));
        info();
      }
    ...
    var fnDoneOne = function fnDoneOne() {
      ...
      if (ctx.countFinished < total) {
        ...
        ctx.statusElement.warn("\u5B8C\u6210\uFF08\u591A\u6267\u884C\u4E86".concat(ctx.countFinished - total, "\u6B21\uFF09"));
        Morebits.wiki.removeCheckpoint();
        ctx.running = false;
      }
    };
  ...
  Morebits.simpleWindow = function SimpleWindow(width, height) {
    ...
    $widget.find('button').each(function (_key, value) {
      value.remove();
    });
  ...
  Morebits.simpleWindow.prototype = {
    ...
    display: function display() {
      if (this.scriptName) {
        ...
        scriptnamespan.textContent = "".concat(this.scriptName, " \xB7 "); // U+00B7 MIDDLE DOT = &middot;
    ...
    addContent: function addContent(content) {
      ...
      var thisproxy = this;
      $(this.content).find('input[type="submit"], button[type="submit"]').each(function (_key, value) {
        value.style.display = 'none';
        var button = document.createElement('button');
        button.textContent = value.hasAttribute('value') ? value.getAttribute('value') : value.textContent || '提交';
      ...
      if (this.buttons.length > 0) {
        delete $(this.content).dialog('widget').find('.morebits-dialog-buttons').empty().append(this.buttons)[0].dataset.empty;
      } else {
