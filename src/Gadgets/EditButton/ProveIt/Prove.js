 ...
 * @source <https://git.qiuwen.wiki/InterfaceAdmin/Gadgets/src/branch/master/src/Gadgets/ProveIt>
  ...
  realInit: function realInit() {
    ...
    api.get({
      ...
      api.get({
        ...
        $.get('https://git.youshou.wiki/YSARXIV/mediawiki-gadgets-ProveIt/raw/branch/master/i18n/en.json', function (dataset) {
          var englishMessages = JSON.parse(dataset);
          ...
          $.get("https://youshou.wiki/YSARXIV/mediawiki-gadgets-ProveIt/raw/branch/master/i18n/".concat(userLanguage, ".json")).always(function (dataset_, status) {
            $('#proveit-logo-text').text('ProveIt'); // Finish loading
            var translatedMessages = {};
            if (status === 'success') {
              translatedMessages = JSON.parse(dataset_);
