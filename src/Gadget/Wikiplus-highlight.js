 ...
 * @source <https://github.com/bhsd-harry/Wikiplus-highlight/blob/main/main.js>
 */
...
(async () => {
	...
	const REPO_CDN = `gh/bhsd-harry/Wikiplus-highlight@${majorVersion}`;
	const {
		config: {
			values: {
				wgPageName: page,
	...
	const EXPIRED = !(SITE_SETTINGS.time > Date.now() - 864e2 * 1e3 * 30);
	...
	const /** @type {addon[]} */ options = [
			...
			{option: 'showTrailingSpace', addon: 'trailingspace'},
			{
				...
				/** @implements */ complex: (mode, json) =>
					mode === 'mediawiki' || json ? {bracketRegex: /[[\]{}]/} : true,
			},
			{
	...
	const handleContextMenu = (doc, mode) => {
		...
		const widget_ = getSysnonyms('widget');
		$wrapper.on('contextmenu', ({pageX, pageY}) => {
			...
			if (/\bmw-template-name\b/.test(curType)) {
				...
				index < 2 ||
				!/\bmw-parserfunction-delimiter\b/.test(tokens[index - 1].type) ||
			...
			} else if (widget_.has(parserFunction)) {
	...
	const initMode = (type) => {
		...
		if (type === 'widget' || type === 'html') {
			for (const lang of ['css', 'javascript', 'mediawiki', 'htmlmixed', 'xml']) {
				if (!CM.modes[lang]) {
					scripts = [...scripts, ...(Array.isArray(MODE_LIST[lang]) ? MODE_LIST[lang] : [MODE_LIST[lang]])];
				}
			}
		} else {
			scripts = [...scripts, ...(Array.isArray(MODE_LIST[type]) ? MODE_LIST[type] : [MODE_LIST[type]])];
		}
		// addons
	 ...
	 */
	const getConfig = (aliases) => Object.fromEntries(aliases.map(({alias, name}) => [alias.replace(/:$/, ''), name]));
	/**
	 * 高亮扩展标签内部
	 *
	...
	const renderEditor = async ($target, setting) => {
		...
		if (!setting) {
			// 普通Wikiplus编辑区
			const /** @type {Wikiplus} */ Wikiplus =
					typeof window.Wikiplus === 'object'
						? window.Wikiplus
						: {
			...
			const escToExitQuickEdit = Wikiplus.getSetting('esc_to_exit_quickedit');
			const submit = /** 提交编辑 */ () => {
